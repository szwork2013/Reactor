...
render_profile = function(barcode) {
  return $.get("/records/" + barcode + "?fields=profile.basic", function(record) {
    ...
    $('.sex').html(record.profile.basic.sex === 'male' ? '男' : '女');
    return $('.age').html(record.profile.basic.age);
  });
};

render_departments = function(barcode) {
  return $.get("/records/" + barcode + "/status", function(departments) {
    var c, content, d, ids, room_id, rooms, _i, _len, _ref;

    content = [];
    departments = departments.filter(function(d) {
      var _ref, _ref1;

      return ((_ref = d.status) === '未完成' || _ref === '未采样' || _ref === '待检验' || _ref === '放弃' || _ref === '延期') && ((_ref1 = d.name) !== '尿常规' && _ref1 !== '便潜血' && _ref1 !== '便隐血');
    });
    ...
    content = (function() {
      ...
      for (room_id in rooms) {
        ...
        if (departments.some(function(d) {
          ...
          return (_ref1 = d.status) === '未完成' || _ref1 === '未采样' || _ref1 === '待检验';
        })) {
          c.status = 'incomplete';
          c.index = 1;
