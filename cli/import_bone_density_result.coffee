...
>>>>>>> a9f64cff12e6edcd89ab34700d4520f2aa227971
  {Record, Department} = models
  Record.find({appeared: {'$gte': '2014-01-01'}})
  .select('barcode')
  .sort('barcode')
  .exec (error, records) ->
    ...
    tasks = barcodes.map (barcode) ->
      (callback) ->
        Record.barcode barcode, (error, record) ->
          ...
          department = _.find record.departments, (d) -> d.name is '骨密度' and d.status is '已完成'
          return callback() unless department
          found_item_value = JSON.parse department.items[0].value
          original_department = Department.clone '骨密度'
          original_item = original_department.items_hash['骨密度']
          item =
            _id: original_item._id
            name: '骨密度'
            category: original_item.category
            value: department.items[0].value
          value = found_item_value.OI
          ...
          single_record_entries =
            barcode: barcode
            entries: [item]
          Record.import_single_record_entries single_record_entries, '骨密度', '周荔'
