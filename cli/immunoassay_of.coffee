...
require('../models') 'hswk.healskare.com', (error, models) ->
  ...
  .exec (error, department) ->
    ...
    Record.find({'profile.source': date}).select('barcode departments.name departments.status departments.items.status').exec (error, records) ->
      # console.log records.length, records[0].departments
      valid_barcodes = records.filter((r)->r.departments.some((d) -> d.name is '生化检验' and d.items.some (i) -> i.status in ['已完成'])).map((r) -> r.barcode)
      console.log valid_barcodes.length
      # return
      # return
      # console.log department
      sampling_name = "黄色采血管（2）" # department.required_samplings[0]
      # 免疫科室所有项目数组
        ...
        Product.get_combos_by_orderids order_ids, (error, order_combos) ->
          ...
          Product.aggregate commands, (error, order_items) ->
            ...
            Product.enterprise_ids (error, ids) ->
              ...
              for record in records
                continue unless record._id in valid_barcodes
