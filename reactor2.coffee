...
.end ({body: {redis: {host, port}}}) ->
  ...
  wss.on 'connection', (ws) ->
    ...
    ws.on 'message', (message) =>
      ...
      #SUBSCRIBE:CHECK_SITUATIONS_INIT:#{departments}:#{app.user._id}
      if messages[1] is 'CHECK_SITUATIONS_INIT'
        ws.departments = messages[2].split(',')
      websockets[ws.page_source]?.push ws
      console.log global.websockets, 'global.websockets'
      models host, (error, models) =>
        ...
        console.log Record?[messages?[1]], 'Record?[messages?[1]]'
  ...
  redis_client.on "message", (channel, message) ->
    ...
    if topic is 'BIOCHEMISTRY_STATUS_CHANGE' and date is message?[0]?.date
      for websocket in websockets[topic_to_socket[topic]] when websocket.host is host
        websocket.send JSON.stringify message
    else if topic is 'CHECK_SITUATIONS_CHANGE'
      for websocket in websockets[topic_to_socket[topic]] when websocket.host is host
        console.log message.names, 'message.names'
        console.log message.transfer_data, 'message.transfer_data'
        console.log websocket.departments, 'websocket.departments'
        if websocket.departments.every((name) -> name in message.names)
