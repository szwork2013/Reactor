...
app.get '/batches/:batch_id/configurations_brochure', (req, res) ->
  ...
  .exec (error, batch) ->
    ...
    user_id = batch.registration?.user_id or '4e5bb37258200ed9aabc5d90'
    console.log '-------------------->', user_id
        ...
        .exec (error, product_combos) ->
          ...
          for configuration in product.configurations
            configuration.market_price = configuration.combos.reduce (memo, combo_id) ->
              memo += small_combo_price[combo_id] or 0
              memo
            , 0
          ...
          res.render 'batch_products_manual', batch_product
