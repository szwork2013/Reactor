...
app.get '/batches/:batch_id/configurations_brochure2.pdf', (req, res) ->
  {batch_id} = req.params
  {Batch, Record}    = req.models
  url = "http://#{req.host}:5124/batches/" + batch_id + '/configurations_brochure2'
  path = __dirname + '/../../public/pdfs/batch_configurations2/' + batch_id + '.pdf'
  options =[
    url
    path
  ]
  grep = exec 'osascript html2pdf2 ' + options.join(' '), {cwd: __dirname + '/../../utils', timeout: 50000}, (err, stdout, stderr) =>
    ...
    .exec (error, batch) ->
      ...
      res.sendfile('./public/pdfs/batch_configurations2/' + batch_id + '.pdf')
