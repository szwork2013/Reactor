...
app.get '/batches/:batch/records_results', authorize('doctor', 'admin'), (req, res) ->
  {Record} = req.models
  {batch} = req.params
    ...
    tasks = barcodes.map (barcode) ->
      (callback) ->
        ...
        .exec (error, record) ->
          ...
          return callback() if record.profile?.name.match(/(测试|盛保善)/) or record.profile?.division?.match(/自费/)
