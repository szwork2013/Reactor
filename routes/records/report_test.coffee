...
app.get /^\/reports_test\/(\w+)(?:\.([pdf]+))?$/, authorize('admin', 'doctor', 'nurse'), (req, res) ->
  barcode = req.params[0]
  pdf = req.params[1]
  {Record} = req.models
  if pdf and pdf is 'pdf'
    Record.summarization barcode, (error, filename) ->
      return res.send error if error
      Record.create_pdf barcode, (error) ->
        return res.send error if error
        res.set "Content-Disposition", "inline; filename*=utf-8''#{encodeURIComponent(filename)}.pdf"
        res.sendfile('./public/pdfs/reports/' + barcode + '.pdf')
  else
    Record.barcode barcode, {paid_all: on}, (error, record) ->
      ...
      record.get_record_combos (error, {laboratory_combos, notes}) ->
        ...
        package_name = _.find(record.orders, (order) -> order.category is 'package')?.name or ''
        record.orders = record.orders.sort((a, b) -> if [2,0,1].indexOf(a.paid) > [2,0,1].indexOf(b.paid) then 1 else -1)
        order_names  = record.orders.map((order) -> order.name.replace(/([\（\）]|[\:\：\w]+)/g, ($0) -> "<span>"+ $0 + "</span>"))
        ...
        report_data["biochemistry_auditor"] = record.biochemistry?.audit?.name if record.biochemistry?.audit?._id
        ...
        if report_data.tct_report
          ...
          sample_date = moment(found_tct_sample?.sample?.date_string)
          report_data.tct_report.sample_date   = sample_date.format('YYYY年MM月') + sample_date.dates() + '日'
          report_data.tct_report.sample_doctor = found_fuke?.checking?.finished?.name
        ...
        res.render 'report_test', report_data
